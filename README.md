...
- **Design system „Liquid Glass" (iOS 26 / One UI 9).** Tokens CSS (`--color-*`, `--radius-*`, `--shadow-*`), `<PageHero>` canonic, butoane glass `<Button>`, dark mode complet, bottom nav mobil flotant + navbar desktop. **Intensitatea sticlei e controlabilă de utilizator** — `--glass-intensity` conduce alfa + blur pe toate straturile `lc-glass`, reglabilă din **`/setari`** alături de toggles de accesibilitate (redu transparența, text mai mare, redu mișcarea, semne `+/−` pentru daltonism). Readability-first (text niciodată dependent de ce se vede prin sticlă), WCAG AA, cu fallback-uri `forced-colors` / `prefers-contrast` / `prefers-reduced-transparency`.
