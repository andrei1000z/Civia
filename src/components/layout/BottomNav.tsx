...
export function BottomNav() {
  ...
  const isActive = (href: string) => {
    ...
    // (Știri). Scoatem primul segment scurt (slug județ 1–2 litere) și comparăm.
  ...
  // element la deschidere, Tab ciclează în sheet, focusul revine la închidere).
  useEffect(() => {
    if (!open) return;
    ...
    <>
      ...
      <div className="lg:hidden fixed inset-x-0 bottom-0 z-[65] flex flex-col items-center px-3 pb-[calc(env(safe-area-inset-bottom,0px)+0.55rem)] pointer-events-none">
        ...
        <div
          ...
          aria-modal="true"
          aria-label="Meniu"
          // inert când e închis: scoate X/search/linkurile din tab-order +
          // din arborele de accesibilitate (nu doar opacity-0).
          ...
          <div className="p-4 max-h-[64vh] overflow-y-auto overscroll-contain">
            ...
            <div className="flex items-center justify-between mb-3">
              ...
              <button
                ...
                className="w-11 h-11 grid place-items-center rounded-full bg-[var(--color-surface)]/60 text-[var(--color-text)] hover:bg-[var(--color-surface)] transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-[var(--color-primary)]"
              >
              ...
              {SHEET_LINKS.map((l, i) => {
                ...
                return (
                  <Link
                    ...
                    className={cn(
                      "flex flex-col items-center justify-center gap-1.5 py-3 px-1.5 rounded-2xl text-center transition-all duration-300 active:scale-95 focus:outline-none focus-visible:ring-2 focus-visible:ring-[var(--color-primary)] motion-reduce:!transition-none motion-reduce:!delay-0",
        ...
        >
          ...
          {TABS.map((t) => {
            ...
            return (
              ...
              >
                <t.Icon
                  size={21}
                  className={cn("transition-colors duration-300", active ? "text-[var(--color-primary-hover)] dark:text-[var(--color-primary)]" : "text-[var(--color-text-muted)]")}
                  aria-hidden="true"
                />
                <span className={cn("text-[10px] font-semibold leading-none transition-colors duration-300", active ? "text-[var(--color-primary-hover)] dark:text-[var(--color-primary)]" : "text-[var(--color-text-muted)]")}>
                  {t.label}
                </span>
              </Link>
            );
          ...
          >
            <MenuIcon
              size={21}
              className={cn("transition-colors duration-300", open ? "text-[var(--color-primary-hover)] dark:text-[var(--color-primary)]" : "text-[var(--color-text-muted)]")}
              aria-hidden="true"
            />
            <span className={cn("text-[10px] font-semibold leading-none transition-colors duration-300", open ? "text-[var(--color-primary-hover)] dark:text-[var(--color-primary)]" : "text-[var(--color-text-muted)]")}>
              Meniu
            </span>
          </button>
        </nav>
